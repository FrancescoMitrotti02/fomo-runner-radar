...
import requests, sqlite3, time, math, json
...
with st.sidebar:
    ...
    base_url = st.text_input("API base", value=DEFAULT_BASE)
    api_key = st.text_input("API key", type="password", placeholder="fsk_live_…")
    st.caption("La chiave resta nella sessione Streamlit e non viene salvata nel database.")
...
if do_scan:
    if not api_key:
        st.error("Inserisci la API key FomoScan.")
    else:
        ts = int(time.time())
        total = 0
        errors = []
