...
import pandas as pd
from watch_engine import ensure_watch_db, process_watchlist
from collector import start_background, INTERVAL as SCAN_INTERVAL
...
def init_db():
    ...
    """)
    # Indici fondamentali: il DB cresce ogni 60s. Senza questi, ogni Runner Score
    # finisce per scansionare migliaia di snapshot e la UI può sembrare bloccata.
    cur.execute("CREATE INDEX IF NOT EXISTS idx_metrics_token_ts ON token_metrics(token_address, ts)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_board_token_board_ts ON board_snapshots(token_address, board, ts)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_board_board_ts ON board_snapshots(board, ts)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_thesis_token_ts ON thesis_snapshots(token_address, ts)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_tokens_first_seen ON tokens(first_seen)")
    con.commit()
    con.close()
...
# Avvia UNA sola istanza persistente del collector nel processo Streamlit.
# st.cache_resource sopravvive ai rerun della pagina e impedisce thread duplicati.
@st.cache_resource
def _start_fomo_collector_once():
    return start_background()

_collector_thread = None
_collector_error = None
try:
    if os.getenv("FOMOSCAN_API_KEY", "").strip():
        _collector_thread = _start_fomo_collector_once()
    else:
        _collector_error = "FOMOSCAN_API_KEY non presente nell'ambiente Render"
except Exception as e:
    _collector_error = str(e)
...
def get_tokens(limit=None):
    con = db()
    sql = "SELECT * FROM tokens ORDER BY first_seen DESC"
    params = ()
...
def get_watchlist():
    con = db()
    try:
        df = pd.read_sql_query("SELECT * FROM watchlist ORDER BY runner_score DESC, risk_score ASC, promoted_at DESC", con)
    except Exception:
        df = pd.DataFrame()
    con.close()
    return df
...
# Il collector esegue la SCANSIONE FOMO vera ogni FOMO_SCAN_INTERVAL secondi.
# La UI NON forza reload: il collector lavora indipendentemente dalla pagina.
UI_REFRESH_SECONDS = max(30, int(os.getenv("FOMO_UI_REFRESH", str(SCAN_INTERVAL))))
collector_ok = _collector_thread is not None and _collector_thread.is_alive()
status_text = "COLLECTOR 24/7 ATTIVO" if collector_ok else "COLLECTOR NON ATTIVO"
status_icon = "🟢" if collector_ok else "🔴"
components.html(
    f"""
    <div style="font-family:system-ui,-apple-system,sans-serif;padding:12px 16px;border-radius:16px;background:linear-gradient(90deg,#3b0764,#7e22ce,#c026d3);color:white;box-shadow:0 6px 20px rgba(192,38,211,.22);margin:0 0 8px 0;">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;">
        <div>
          <div style="font-size:12px;opacity:.86;font-weight:800;letter-spacing:.08em;">{status_icon} {status_text}</div>
          <div style="font-size:18px;font-weight:900;">📡 Prossima scansione FOMO tra</div>
          <div style="font-size:12px;opacity:.82;margin-top:2px;">La scansione avviene sul server ogni {SCAN_INTERVAL}s, anche senza pagina aperta.</div>
        </div>
        <div id="fomo-countdown" style="min-width:76px;text-align:center;font-size:27px;font-weight:900;background:rgba(255,255,255,.16);border:1px solid rgba(255,255,255,.25);padding:8px 12px;border-radius:14px;">{SCAN_INTERVAL}s</div>
      </div>
    </div>
    <script>
      let left = {SCAN_INTERVAL};
      const el = document.getElementById('fomo-countdown');
      setInterval(() => {{
        left -= 1;
        if (left <= 0) {{
          el.textContent = 'SCAN';
          left = {SCAN_INTERVAL};
          setTimeout(() => {{ el.textContent = left + 's'; }}, 700);
          return;
        }}
...
if _collector_error:
    st.error(f"Collector automatico non avviato: {_collector_error}")
...
if do_scan:
    if not api_key:
        ...
        try:
            ...
            st.warning(f"Watchlist: analisi rimandata ({e})")
        if diagnostics:
...
# Dashboard iniziale volutamente leggero.
# NON calcoliamo Runner Score qui: la pagina deve comparire subito anche con mesi di storico.
try:
    con = db()
    token_count = con.execute("SELECT COUNT(*) FROM tokens").fetchone()[0]
    snap_count = con.execute("SELECT COUNT(*) FROM board_snapshots").fetchone()[0]
    last_scan_ts = con.execute("SELECT MAX(ts) FROM board_snapshots").fetchone()[0]
    con.close()
    q1, q2, q3 = st.columns(3)
    q1.metric("Token visti", int(token_count or 0))
    q2.metric("Snapshot", int(snap_count or 0))
    if last_scan_ts:
        age = max(0, int(time.time()) - int(last_scan_ts))
        q3.metric("Ultima scansione", f"{age}s fa")
    else:
        q3.metric("Ultima scansione", "—")
except Exception:
    pass

SECTIONS = [
    "🚨 Early Runner Radar",
    "👁️ Sotto osservazione",
...
# IMPORTANTE: le st.tabs eseguono il codice di tutte le schede a ogni rerun.
# Con un database che cresce ogni 60s questo può bloccare la UI.
# Il radio rende solo la sezione selezionata e lascia il collector indipendente.
section = st.radio("Sezione", SECTIONS, index=4, horizontal=True, label_visibility="collapsed")

# ---------------------------
# Early Runner Radar
# ---------------------------
if section == "🚨 Early Runner Radar":
    # I runner sono per definizione recenti: evitiamo di ricalcolare l'intero archivio storico.
    toks = get_tokens(limit=150)
    if toks.empty:
...
# ---------------------------
# Sotto osservazione / anti-frode
# ---------------------------
if section == "👁️ Sotto osservazione":
    st.subheader("👁️ Sotto osservazione")
    st.caption("Un token entra qui automaticamente quando il Runner Score raggiunge 72/100. I controlli riducono il rischio informativo, ma non possono certificare che un token sia sicuro o non sia una truffa.")
    watch = get_watchlist()
    if watch.empty:
        st.info("Nessun Early Runner è ancora stato promosso sotto osservazione.")
    else:
        rows=[]
        for _,w in watch.iterrows():
            try: flags=json.loads(w.get("fraud_flags_json") or "[]")
            except: flags=[]
            severe=sum(1 for f in flags if f.get("severity") in ("CRITICAL","HIGH"))
            minor=sum(1 for f in flags if f.get("severity") in ("MEDIUM","LOW"))
            rows.append({
                "Token": w.get("symbol") or w.get("name") or "?",
                "Runner Score": None if pd.isna(w.get("runner_score")) else round(float(w.get("runner_score")),1),
                "Rischio": w.get("risk_level") or "IN ANALISI",
                "Risk score": None if pd.isna(w.get("risk_score")) else round(float(w.get("risk_score")),0),
                "Problemi gravi": severe,
                "Problemi minori": minor,
                "Chain": w.get("chain") or "?",
                "Stato verifica": w.get("status") or "IN ANALISI",
                "Promossa": pd.to_datetime(w.get("promoted_at"),unit="s",utc=True) if w.get("promoted_at") else None,
                "CA": w.get("token_address")
            })
        wdf=pd.DataFrame(rows).sort_values(["Runner Score","Risk score"],ascending=[False,True],na_position="last").reset_index(drop=True)
        wdf.insert(0,"#",range(1,len(wdf)+1))
        st.dataframe(wdf,use_container_width=True,hide_index=True)

        labels=[]
        for _,w in watch.iterrows():
            labels.append(f"{w.get('symbol') or w.get('name') or '?'} — {str(w.get('token_address'))[:10]}…")
        selected=st.selectbox("Dettaglio controlli", labels)
        w=watch.iloc[labels.index(selected)]
        try: flags=json.loads(w.get("fraud_flags_json") or "[]")
        except: flags=[]
        try: social=json.loads(w.get("social_json") or "{}")
        except: social={}
        c1,c2,c3,c4=st.columns(4)
        c1.metric("Runner Score",f"{float(w.get('runner_score') or 0):.1f}/100")
        c2.metric("Rischio",str(w.get("risk_level") or "IN ANALISI"))
        c3.metric("Chain",str(w.get("chain") or "?"))
        c4.metric("Ultimo controllo", datetime.fromtimestamp(int(w.get("last_checked") or 0),tz=timezone.utc).strftime("%H:%M UTC") if w.get("last_checked") else "in attesa")

        if not flags:
            if str(w.get("status")) == "IN ANALISI": st.info("Controlli anti-frode in corso.")
            else: st.success("✅ Nessun problema rilevato dai controlli automatici disponibili.")
        else:
            st.markdown("#### 🛡️ Verifiche anti-frode")
            for f in flags:
                sev=f.get("severity","?"); txt=f.get("text",""); src=f.get("source","")
                if sev in ("CRITICAL","HIGH"): st.error(f"{sev} · {txt} — {src}")
                elif sev=="MEDIUM": st.warning(f"{sev} · {txt} — {src}")
                else: st.info(f"{sev} · {txt} — {src}")

        st.markdown("#### 🌐 Percezione web e social")
        st.write(w.get("public_summary") or "In attesa della verifica reputazionale.")
        if social and not social.get("skipped"):
            s1,s2,s3,s4=st.columns(4)
            s1.metric("Footprint",int(social.get("footprint") or 0))
            s2.metric("Web",len(social.get("web_results") or []))
            s3.metric("X",len(social.get("x_results") or []))
            s4.metric("Reddit",len(social.get("reddit_results") or []))
            dex_socials=social.get("dex_socials") or []
            dex_websites=social.get("dex_websites") or []
            if dex_socials or dex_websites:
                with st.expander("Link ufficiali/social rilevati da DexScreener"):
                    st.json({"socials":dex_socials,"websites":dex_websites})
            results=(social.get("web_results") or [])+(social.get("x_results") or [])+(social.get("reddit_results") or [])
            if results:
                with st.expander("Risultati usati per il riepilogo automatico"):
                    for r in results[:15]:
                        st.write(f"• **{r.get('title','')}** — {r.get('snippet','')}")
        elif social.get("skipped"):
            st.warning("La ricerca reputazionale finale è stata saltata perché sono già emersi rischi tecnici materiali.")

        st.caption("Fonti automatiche: dati FomoScan + DexScreener; per token EVM supportati vengono interrogati anche GoPlus e Honeypot.is. La ricerca web/social è best-effort e può non indicizzare contenuti molto recenti o privati.")

# ---------------------------
# Winners after X time
# ---------------------------
if section == "🏆 Migliori dopo X tempo":
...
if section == "📈 Trending FOMO":
    render_board("trending")
if section == "💎 Most-held":
    render_board("most-held")
if section == "🎓 Graduated":
    render_board("graduated")

# ---------------------------
# Thesis
# ---------------------------
if section == "🧠 Thesis Feed":
    th=get_theses()
    if th.empty:
...
if section == "📚 Storico":
    ...
    st.write("Token sotto osservazione:",len(get_watchlist()))
...
Quando un token raggiunge **72/100 (Early Runner)** viene promosso automaticamente in **Sotto osservazione**, dove passa attraverso controlli di liquidità/mercato, contract risk quando disponibile e una verifica reputazionale web/social solo se non emergono rischi tecnici materiali.

**Runner Score e Risk score non sono probabilità di profitto né certificazioni di sicurezza.** Servono a ordinare i candidati e a rendere visibili anomalie e segnali di rischio.
""")
