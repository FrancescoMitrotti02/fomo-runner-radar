...
# Refresh visivo della dashboard. Il collector continua a usare FOMO_SCAN_INTERVAL
# per la frequenza delle scansioni; questo countdown ricarica la UI per mostrare i dati nuovi.
UI_REFRESH_SECONDS = max(30, int(os.getenv("FOMO_UI_REFRESH", os.getenv("FOMO_SCAN_INTERVAL", "60"))))
components.html(
    f"""
    <div style="font-family:system-ui,-apple-system,sans-serif;display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px 16px;border-radius:16px;background:linear-gradient(90deg,#3b0764,#7e22ce,#c026d3);color:white;box-shadow:0 6px 20px rgba(192,38,211,.22);margin:0 0 8px 0;">
      <div>
        <div style="font-size:12px;opacity:.82;font-weight:700;letter-spacing:.08em;">AUTO REFRESH RADAR</div>
        <div style="font-size:18px;font-weight:800;">⚡ Aggiornamento ogni {UI_REFRESH_SECONDS}s</div>
      </div>
      <div id="fomo-countdown" style="min-width:76px;text-align:center;font-size:27px;font-weight:900;background:rgba(255,255,255,.16);border:1px solid rgba(255,255,255,.25);padding:8px 12px;border-radius:14px;">{UI_REFRESH_SECONDS}s</div>
    </div>
    <script>
      let left = {UI_REFRESH_SECONDS};
